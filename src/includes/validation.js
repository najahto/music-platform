    ...
    configure({
      generateMessage: (context) => {
        const messages = {
          ...
          county_excluded: `Due to restrictions, we do not accept users from this location ${context.field}`,
          passwords_mismatch: "The password don't match",
          tos: 'You must accept the Terms of service',
        };

        const message = messages[context.rule.name] ? messages[context.rule.name]
